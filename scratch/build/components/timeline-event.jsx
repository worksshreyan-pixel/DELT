...
import { FileText, FilePlus, CheckCircle2, DollarSign, Send, Eye, Mail, ArrowLeftRight, Upload, Unlock, Flag, XCircle, } from 'lucide-react';
...
export function TimelineEvent(_a) {
    var event = _a.event, isLast = _a.isLast;
    var config = eventConfig[event.type] || eventConfig.message_sent;
    var Icon = config.icon;
    return (<motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }} className="flex gap-3">
...
export function Timeline(_a) {
    ...
    return (<div className="space-y-0">
      {events.map(function (event, i) { return (<TimelineEvent key={event.id} event={event} isLast={i === events.length - 1}/>); })}
    </div>);
}
