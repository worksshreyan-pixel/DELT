...
export function FileCard(_a) {
    var file = _a.file, locked = _a.locked;
    var Icon = getFileIcon(file.type);
    return (<motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className={cn('group flex items-center gap-3 rounded-lg border border-border bg-card p-3 transition-colors', locked ? 'opacity-60' : 'hover:bg-accent/50')}>
