...
import { ArrowRight, Check, HardDrive, CreditCard } from 'lucide-react';
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 pb-16">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {PLAN_LIST.map(function (plan, i) { return (<motion.div key={plan.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: i * 0.05 }}>
              <Card className={cn('h-full relative', plan.highlighted && 'border-primary shadow-lg ring-1 ring-primary/20')}>
                ...
                <CardContent className="p-6">
                  ...
                  <ul className="mt-5 space-y-2">
                    {plan.features.map(function (feature) { return (<li key={feature} className="flex items-start gap-2 text-sm">
                  ...
                  <Link href="/signup" className="mt-6 block">
                    <Button className="w-full" variant={plan.highlighted ? 'default' : 'outline'}>
                      {plan.price ? "Choose ".concat(plan.name) : 'Start free'}
                    </Button>
                  </Link>
                </CardContent>
              </Card>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="space-y-2">
                  {STORAGE_ADDONS.map(function (addon) { return (<div key={addon.id} className="flex items-center justify-between rounded-lg border border-border bg-muted/30 px-3 py-2">
      ...
      <section className="border-t border-border bg-muted/20 py-16">
        ...
        ].map(function (faq) { return (<Card key={faq.q}>
