...
var principles = [
    { icon: KeyRound, title: 'Verified access', desc: 'Clients access deals through a private link plus email verification via OTP. No unguessable URL is treated as a security boundary on its own.' },
    { icon: Lock, title: 'Private file storage', desc: 'Deliverables are stored in private buckets, not public URLs. Access is authorized per deal participant and delivered through signed URLs.' },
    { icon: FileCheck, title: 'Payment-aware delivery', desc: 'Files remain locked until payment is confirmed server-side. The client cannot unlock deliverables through frontend state alone.' },
    { icon: Eye, title: 'Participant-based access', desc: 'Only the creator and the invited client can access a deal workspace. Authorization is enforced at the data layer, not by hiding buttons.' },
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-5xl px-4 sm:px-6 pb-16">
        <div className="grid gap-4 sm:grid-cols-2">
          {principles.map(function (p) { return (<Card key={p.title} className="h-full">
      ...
      <section className="border-t border-border bg-muted/20 py-16">
        ...
        ].map(function (item) { return (<div key={item} className="flex items-center gap-3 rounded-lg border border-border bg-card px-4 py-3">
