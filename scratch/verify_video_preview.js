...
// Test of the segment duration and start offset calculation logic
...
async function verify() {
  ...
  const ffmpegStatus = await isFfmpegAvailable();
  ...
  for (const d of testDurations) {
    ...
    // Validations
    if (d <= 15) {
      if (previewDuration !== d) throw new Error(`Failed: expected entire video for ${d}s`);
      if (previewStart !== 0) throw new Error(`Failed: expected start=0 for ${d}s`);
    } else {
