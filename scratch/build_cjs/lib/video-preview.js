...
exports.generateVideoPreview = exports.isFfmpegAvailable = void 0;
...
function getFontPath() {
    ...
    if (fs_1.default.existsSync(winFont)) {
        return winFont;
    }
    const linuxFonts = [
...
function getEscapedFontFileParam() {
    ...
    const localFontPath = path_1.default.join(process.cwd(), localFontName);
    if (!fs_1.default.existsSync(localFontPath) && fs_1.default.existsSync(fontPath)) {
        try {
            fs_1.default.copyFileSync(fontPath, localFontPath);
    ...
    if (fs_1.default.existsSync(localFontPath)) {
...
function getWatermarkFilter() {
    ...
    for (let y = 30; y < 480; y += stepY) {
        ...
        for (let x = 30; x < 854; x += stepX) {
            ...
            drawtextFilters.push(`drawtext=text='${escapedText}':fontcolor=0x464646@0.0:borderw=1.5:bordercolor=0x464646@0.35:fontsize=${fontSize}${fontParam}:x=${x + xOffset}:y=${y}`);
        }
    }
...
async function generateVideoPreview(dealId, fileVersionId, fileId) {
    ...
    try {
        ...
        tempOutPath = path_1.default.join(tempDir, `preview_${dealId}_${fileId}_${Date.now()}.mp4`);
        // Build filtergraph
        ...
        const previewStats = fs_1.default.statSync(tempOutPath);
        ...
        const previewBuffer = fs_1.default.readFileSync(tempOutPath);
        ...
        if (tempOutPath && fs_1.default.existsSync(tempOutPath)) {
            try {
...
exports.generateVideoPreview = generateVideoPreview;
