...
exports.verifyClientSessionToken = exports.verifyDealOtp = exports.requestDealOtp = exports.verifySignupOtp = exports.requestSignupOtp = exports.normalizeEmail = void 0;
...
const inMemorySignupOtpStore = new Map();
// ------------------------------------------------------------------------------
// 1. Creator Signup OTP — Request & Resend
// ------------------------------------------------------------------------------
async function requestSignupOtp(params) {
    ...
    const randomOtp = crypto_1.default.randomInt(100000, 1000000).toString();
    ...
    const emailRes = await (0, email_1.sendOtpEmail)({
        to: normalizedEmail,
        otpCode: randomOtp,
        expiresInMinutes: 10,
...
// ------------------------------------------------------------------------------
async function requestDealOtp(dealToken, rawEmail, traceId) {
    const otpTraceId = traceId || `OTP-REQUEST-${crypto_1.default.randomUUID().slice(0, 8)}`;
...
exports.requestDealOtp = requestDealOtp;
// ------------------------------------------------------------------------------
// 4. Client Deal Access OTP — Verification (Database Backed: deal_otps)
// ------------------------------------------------------------------------------
async function verifyDealOtp(dealToken, rawEmail, inputOtp, traceId) {
    const otpTraceId = traceId || `OTP-VERIFY-${crypto_1.default.randomUUID().slice(0, 8)}`;
    const normalizedEmail = normalizeEmail(rawEmail);
...
exports.verifyDealOtp = verifyDealOtp;
/**
 * Validates a signed Client Session Token.
 */
function verifyClientSessionToken(tokenString, dealToken, rawEmail) {
    ...
    if (sigBuffer.length !== expBuffer.length || !crypto_1.default.timingSafeEqual(sigBuffer, expBuffer)) {
        return false;
    }
    try {
...
exports.verifyClientSessionToken = verifyClientSessionToken;
