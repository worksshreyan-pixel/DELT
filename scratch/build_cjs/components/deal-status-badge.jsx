...
exports.MilestoneStatusBadge = exports.DeliverableStatusBadge = exports.ProposalStatusBadge = exports.PaymentStatusBadge = exports.DealStatusBadge = void 0;
...
function DealStatusBadge({ status, className }) {
    const config = statusConfig[status];
    return (<span className={(0, utils_1.cn)('inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium', config.className, className)}>
      <span className={(0, utils_1.cn)('h-1.5 w-1.5 rounded-full', config.dot)}/>
      {config.label}
    </span>);
}
exports.DealStatusBadge = DealStatusBadge;
const paymentConfig = {
...
function PaymentStatusBadge({ status, className }) {
    ...
    return (<span className={(0, utils_1.cn)('inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium', config.className, className)}>
      {config.label}
    </span>);
}
...
function ProposalStatusBadge({ status, className }) {
    const config = proposalConfig[status];
    return (<span className={(0, utils_1.cn)('inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium', config.className, className)}>
      {config.label}
    </span>);
}
exports.ProposalStatusBadge = ProposalStatusBadge;
...
exports.DeliverableStatusBadge = DeliverableStatusBadge;
...
function MilestoneStatusBadge({ status, className }) {
