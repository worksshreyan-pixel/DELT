...
async function POST(request) {
    try {
        ...
        if (!orderId || !paymentId) {
            return server_1.NextResponse.json({ error: 'Order ID and Payment ID are required' }, { status: 400 });
        }
        const supabase = (0, admin_1.createAdminClient)();
        // Verify signature in live Razorpay mode
        if ((0, env_1.hasRazorpayConfig)() && !demo) {
            const isValid = (0, razorpay_1.verifyRazorpaySignature)(orderId, paymentId, signature);
            if (!isValid) {
                return server_1.NextResponse.json({ error: 'Invalid payment signature' }, { status: 400 });
            }
        }
        // Fetch deal
        ...
        try {
            ...
            // Email to Client
            await (0, email_1.sendPaymentConfirmationEmail)({
        ...
        return server_1.NextResponse.json({ error: error?.message || 'Payment verification failed' }, { status: 500 });
    }
}
exports.POST = POST;
