...
function SignupPage() {
    ...
    const [verifiedSuccess, setVerifiedSuccess] = (0, react_1.useState)(false);
    ...
    const [expiresSeconds, setExpiresSeconds] = (0, react_1.useState)(600); // 10 minutes
          ...
          <div className="w-full max-w-md">
            ...
            <div className="rounded-xl border border-border bg-card p-6 sm:p-8 shadow-sm text-center space-y-5">
              ...
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10">
                {verifiedSuccess ? (<lucide_react_1.CheckCircle2 className="h-7 w-7 text-emerald-600 dark:text-emerald-400"/>) : (<lucide_react_1.KeyRound className="h-7 w-7 text-primary"/>)}
              </div>
              ...
              {statusMessage && (<div className={(0, utils_1.cn)('flex items-center justify-center gap-2 rounded-lg p-2.5 text-xs font-medium', verifiedSuccess
                    ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400'
                    : 'bg-primary/10 text-primary')}>
                  <lucide_react_1.CheckCircle2 className="h-4 w-4 shrink-0"/>
              ...
              {!verifiedSuccess && (<div className="space-y-4 pt-2">
                  <div className="flex justify-center gap-2">
                    {otp.map((digit, idx) => (<input key={idx} ref={(el) => {
                        otpRefs.current[idx] = el;
                    }} type="text" inputMode="numeric" pattern="[0-9]*" maxLength={1} value={digit} onChange={(e) => handleOtpChange(idx, e.target.value)} onKeyDown={(e) => handleOtpKeyDown(idx, e)} onPaste={handleOtpPaste} disabled={verifying || verifiedSuccess} className={(0, utils_1.cn)('h-12 w-11 sm:h-14 sm:w-12 rounded-lg border border-input bg-background text-center text-xl font-semibold shadow-sm transition-colors focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary disabled:opacity-50', digit && 'border-primary/60 bg-muted/20')}/>))}
                  </div>
                  ...
                  <div className="space-y-2 pt-2">
                    <button_1.Button onClick={() => handleVerifyOtp()} disabled={verifying || otp.some((d) => !d) || verifiedSuccess} className="w-full gap-2">
                      {verifying ? 'Verifying...' : 'Verify'}
                      {!verifying && <lucide_react_1.ArrowRight className="h-4 w-4"/>}
                    </button_1.Button>

                    <div className="flex items-center justify-between text-xs pt-1">
                      <button_1.Button variant="ghost" size="sm" onClick={handleResendOtp} disabled={resending || cooldown > 0 || verifiedSuccess} className="text-xs gap-1.5 h-8 px-2 text-muted-foreground hover:text-foreground">
                        <lucide_react_1.RefreshCw className={(0, utils_1.cn)('h-3.5 w-3.5', resending && 'animate-spin')}/>
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <lucide_react_1.User className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
                  <input_1.Input id="name" type="text" placeholder="Alex Morgan" className="pl-9" value={name} onChange={(e) => setName(e.target.value)} required/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {password && (<div className="space-y-1 pt-1">
                    {passwordChecks.map((check, i) => (<div key={i} className="flex items-center gap-1.5 text-xs">
                        <lucide_react_1.Check className={(0, utils_1.cn)('h-3 w-3', check.met ? 'text-emerald-500' : 'text-muted-foreground/40')}/>
              ...
              <div className="flex items-start gap-2 pt-1">
                <input type="checkbox" id="terms" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="mt-1 h-3.5 w-3.5 rounded border-input text-primary focus:ring-primary"/>
                <label_1.Label htmlFor="terms" className="text-xs text-muted-foreground font-normal leading-relaxed">
                  I agree to the{' '}
                  <link_1.default href="/terms" className="text-foreground underline underline-offset-4 hover:text-primary">
                    Terms of Service
                  </link_1.default>{' '}
                  and{' '}
                  <link_1.default href="/privacy" className="text-foreground underline underline-offset-4 hover:text-primary">
                    Privacy Policy
                  </link_1.default>
                </label_1.Label>
              </div>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            ...
            <link_1.default href="/login" className="font-medium text-foreground hover:underline">
              Sign in
            </link_1.default>
          </p>
        </div>
