...
const statusFilters: { label: string; value: PaymentState | 'refunded' | 'all' }[] = [
  { label: 'All', value: 'all' },
...
type TxSort = 'date-desc' | 'date-asc' | 'amount-desc' | 'amount-asc';
...
export default function TransactionsPage() {
  ...
  const [filter, setFilter] = useState<PaymentState | 'refunded' | 'all'>('all');
  const [sortBy, setSortBy] = useState<TxSort>('date-desc');
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Transactions"
        description="Payment history, platform fees, and net revenue payouts."
      />
        ...
        <Card>
          <CardContent className="py-12">
            <EmptyState
              ...
              onAction={transactions.length > 0 ? () => { setSearch(''); setFilter('all'); } : undefined}
            />
          </CardContent>
        </Card>
      ) : (
