...
import { addMessageToStore, addProposalToStore, simulatePaymentInStore } from '@/lib/app-store';
...
export default function ClientDealPage() {
  ...
  // Request Supabase Auth OTP (Single Request Guaranteed)
    ...
    try {
      ...
      if (otpError) {
        ...
        const isRateLimit =
          ...
          errMsg.includes('over_request_rate_limit') ||
      ...
      setError(e.message || 'Network error requesting verification code.');
    } finally {
      setIsSending(false);
    }
  }
  ...
  async function handleVerifyOtp() {
    ...
    try {
      ...
      // Supabase session established! Now verify Deal authorization on the server
      const res = await fetch(`/api/deals/${encodeURIComponent(token)}/verify-access`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      ...
      if (!res.ok || !json.authorized) {
        setError(json.error || 'This account is not authorized for this Deal workspace.');
        setVerifying(false);
        return;
      }
  ...
  function handleOtpChange(idx: number, value: string) {
    ...
    if (value && idx === 5 && newOtp.every((d) => d !== '')) {
      setTimeout(() => handleVerifyOtpDirect(newOtp.join('')), 50);
    }
  }
  ...
  function handleOtpPaste(e: React.ClipboardEvent<HTMLInputElement>) {
    ...
    if (digits.length === 6) {
      setTimeout(() => handleVerifyOtpDirect(newOtp.join('')), 100);
    }
  }

  async function handleVerifyOtpDirect(code: string) {
    if (code.length !== 6) return;
    const targetEmail = (email || dealMeta?.clientEmail || '').trim().toLowerCase();
    if (!targetEmail) {
      setError('Please provide your client email address.');
      return;
    }
...
function ClientPortal({
  ...
  async function handleProposePrice(e: React.FormEvent) {
    ...
    const newProp = addProposalToStore(currentDeal.id, price, proposalReason.trim() || undefined, 'client', clientName);
  ...
  async function handleDownloadFile(filePath: string) {
    ...
    try {
      const res = await fetch('/api/files/signed-url', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dealId: currentDeal.id, filePath, isCreator: false }),
      });
    ...
    try {
      for (const f of allFiles) {
        const res = await fetch('/api/files/signed-url', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ dealId: currentDeal.id, filePath: f.path, isCreator: false }),
        });
        if (res.ok) {
          const { signedUrl } = await res.json();
          if (signedUrl) {
                          ...
                          return (
                            <ChatMessageItem key={msg.id} message={msg} isCurrentUser={msg.senderRole === 'client'} showAvatar={showAvatar}>
                              <div className="max-w-sm">
                                <PriceProposalCard proposal={proposal} currency={currentDeal.currency} perspective="client" />
                              </div>
                            </ChatMessageItem>
                          );
                        }
