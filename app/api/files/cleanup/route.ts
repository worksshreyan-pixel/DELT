  ...
  try {
    ...
    if (serviceKey && authHeader !== `Bearer ${serviceKey}`) {
      ...
      if (process.env.NODE_ENV === 'production') {
        return NextResponse.json({ error: 'Unauthorized system service call' }, { status: 401 });
      }
    }
    ...
    if (fileVersions && fileVersions.length > 0) {
      for (const version of fileVersions) {
        ...
        const deal = version.deals as any;
    ...
    return NextResponse.json(
      { error: error?.message || 'Internal server error executing cleanup' },
      { status: 500 }
    );
  }
}
